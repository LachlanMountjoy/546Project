...
async function getBidderByItem(itemId) {
    if(!itemId) throw 'you must provide an itemId to search for ';
    const bidderCollection = await bidders();
    const bidder = await bidderCollection.find({ itemId: itemId }).toArray();
    if(bidder === null) throw 'no bidder with the id ';
    return bidder;
}
